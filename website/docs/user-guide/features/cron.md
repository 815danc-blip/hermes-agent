...
- **Per-job pin** — set by *you* via the dashboard, `hermes cron create/edit --model … --provider …`, or by editing `~/.hermes/cron/jobs.json`. Once set, it sticks until you change it. The agent's `cronjob_manage` tool cannot set or change per-job models — inference pins are user-owned.
...
The agent-facing `cronjob_manage` tool accepts the same action (`action=resnap job_id=<id>` or
